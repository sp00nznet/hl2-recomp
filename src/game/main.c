...
/* Give the engine back its own console.
 *
 * Two facts about the retail Xbox image: the spew gate at 0x0081598C ships
 * as 1, so Msg() returns before formatting anything; and the sink pointer at
 * 0x00815990 is never initialised, because both functions that would install
 * a sink (sub_005960C0, sub_00596710) have no callers in this build. The
 * shipped game is silent by construction.
 *
 * Clearing the gate and pointing the sink at 0x00596080 -- whose stub body is
 * overridden natively in recomp_manual.c -- routes every Msg, Warning and
 * DevMsg the engine already formats to stderr. This is the engine reporting
 * on itself, which beats anything observed from outside it.
 *
 * It does modify guest state, so HL2_SPEW=0 turns it off and restores an
 * untouched boot.
 */
/* Watch the D3D push-buffer fence.
 *
 * The device struct is statically allocated at 0x0061C940, so the whole
 * chain the guest spins on has fixed addresses:
 *
 *     regs  = MEM32(device + 0x1C20)   the NV2A register block
 *     GET   = MEM32(regs   + 0x44)     how far the GPU has consumed
 *     PUT   = MEM32(device + 0x24)     how far the title has written
 *     LIMIT = MEM32(device + 0x28)
 *
 * sub_00612430 waits for GET to reach PUT. Whether the ack thread's
 * PUT->GET copy in the 0xFD000000 aperture reaches this GET depends on
 * where regs actually points, which is decided at runtime -- so print it.
 */
static DWORD WINAPI hl2_gpu_fence_trace(LPVOID unused)
{
    uint32_t last_get = 0xFFFFFFFFu, last_put = 0xFFFFFFFFu;
    int i;

    (void)unused;
    for (i = 0; i < 60; i++) {
        uintptr_t base = (uintptr_t)g_xbox_mem_offset;
        uint32_t regs, get = 0, put, limit;

        Sleep(2000);
        if (!base)
            continue;
        regs  = *(uint32_t *)(base + 0x0061C940u + 0x1C20u);
        put   = *(uint32_t *)(base + 0x0061C940u + 0x24u);
        limit = *(uint32_t *)(base + 0x0061C940u + 0x28u);
        if (regs)
            get = *(uint32_t *)(base + regs + 0x44u);
        if (get == last_get && put == last_put)
            continue;
        last_get = get; last_put = put;
        fprintf(stderr,
                "  [GPU] regs=0x%08X GET=0x%08X PUT=0x%08X LIMIT=0x%08X%s\n",
                regs, get, put, limit,
                (regs >= 0xFD000000u && regs < 0xFE000000u)
                    ? "  (regs in NV2A aperture)" : "  (regs NOT in aperture)");
        fflush(stderr);
    }
    return 0;
}

static void hl2_enable_engine_spew(void)
{
    const char *opt = getenv("HL2_SPEW");

    /* Opt-in: forcing the gate open sends engine text to the debug monitor
     * (XBX_SendRemoteCommand), which a retail image has no XBDM to answer.
     * Default off keeps boot faithful; [GUEST] DbgPrint is the channel that
     * actually works. */
    if (!opt || opt[0] == '0')
        return;
    ...
    /* main.c reaches guest memory through g_xbox_mem_offset, as elsewhere
     * in this file; the MEM8/MEM32 macros belong to the generated code. */
    *(uint8_t *)((uintptr_t)g_xbox_mem_offset + 0x0081598Cu) = 0;
    *(uint32_t *)((uintptr_t)g_xbox_mem_offset + 0x00815990u) = 0x00596080u;

    printf("Engine spew enabled; HL2_SPEW=0 to disable.\n");
    fflush(stdout);
}
...
/* Sample every thread in the process, not just the one that armed this.
 *
 * The runtime is genuinely multithreaded now -- the title spawns workers
 * through PsCreateSystemThreadEx -- so a hang is no longer necessarily the
 * main thread's. Half-Life 2 blocks the main thread in a correct INFINITE
 * NtWaitForMultipleObjectsEx while a *worker* burns CPU, and sampling only
 * the arming thread reports the wait, which is the one thread behaving
 * properly.
 *
 * Toolhelp rather than a registry of handles: it needs no cooperation from
 * the spawn path and cannot go stale.
 */
static void sample_all_threads(void)
{
    ...
    if (Thread32First(snap, &te)) {
        do {
            ...
            if (te.th32OwnerProcessID != self_pid || te.th32ThreadID == me)
                continue;
            ...
            if (GetThreadTimes(th, &c, &e, &k, &u)) {
                ...
                busy = (ku.QuadPart + uu.QuadPart) / 10000;   /* ms */
            }
            memset(&ctx, 0, sizeof(ctx));
            ctx.ContextFlags = CONTEXT_CONTROL;
            if (SuspendThread(th) != (DWORD)-1) {
                if (GetThreadContext(th, &ctx)) {
                    fprintf(stderr, "  tid %-6lu cpu %6llums  ",
...
static DWORD WINAPI hang_watchdog(LPVOID arg)
{
    ...
    for (sample = 0; sample < 4; sample++) {
        ...
        if (GetThreadContext(g_guest_thread, &ctx)) {
            ...
            print_host_symbol((void *)(uintptr_t)ctx.Rip);
            /* And every other thread: the arming thread is often the one
             * behaving correctly while a worker spins. */
            sample_all_threads();
            /* Indirect calls since the last sample. Plain global, not TLS,
             * so it is readable from here -- and it is the difference
             * between a tight spin and something that is merely slow, which
             * a symbol alone cannot tell you. */
            {
                static unsigned long long prev;
                unsigned long long now = g_icall_count;
                fprintf(stderr, "  indirect calls: %llu (+%llu since last)\n",
                        now, now - prev);
                prev = now;
            }
...
int main(int argc, char **argv)
{
    ...
    /* The GPU fence D3D waits on.
     *
     * sub_00612430 reserves pushbuffer space and spins until the GPU has
     * consumed enough of it:
     *
     *     mov edx, [esi+0x30]    ; esi = the device at 0x0061C940
     *     mov esi, [esi+0x2c]    ;   +0x2C = what we have submitted
     *   loop:
     *     mov ecx, [edx]         ;   *(+0x30) = what the GPU has consumed
     *     ...
     *     jb  loop
     *
     * The fence is the 96-byte contiguous block the device points at, which
     * on hardware the GPU writes as it drains the pushbuffer. Nothing here
     * executes the pushbuffer, so everything submitted is complete and
     * advancing the fence is the same honest acknowledgement the NV2A
     * register tables already make.
     *
     * 0x0061EDE8 is the global holding the device pointer; the chain is
    ...
    if (getenv("HL2_GPU_TRACE")) {
        HANDLE th = CreateThread(NULL, 0, hl2_gpu_fence_trace, NULL, 0, NULL);
