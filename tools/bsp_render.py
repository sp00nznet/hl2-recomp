...
def render(verts, faces, out_path, eye=None, angles=None):
...
def main():
    ...
    version, verts, faces = load_geometry(sys.argv[1])
    data = Path(sys.argv[1]).read_bytes()
    eye, angles = player_start(data)
    drawn = render(verts, faces, sys.argv[2], eye, angles)
    where = (f"from info_player_start {eye[0]:.0f},{eye[1]:.0f},{eye[2]:.0f} "
             f"yaw {angles[1]:.0f}" if eye else "from outside")
    print(f"VBSP v{version}: {len(verts)} vertices, {len(faces)} faces, "
          f"{drawn} drawn {where} -> {sys.argv[2]}")
