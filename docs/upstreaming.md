...
| `swizzle_offset` Morton addressing | **fixed upstream**, `src/d3d/d3d8_swizzle.h` | It spread both coordinates onto even bit positions and masked, but the Y mask selects odd ones, so the Y term was almost always zero -- 261,632 of 262,144 coordinates collided at 512x512. It had never had a caller until a per-pixel sampler needed it, which is why it shipped wrong. Now a general bit deposit against `xbox_swizzle_masks`, the same generator the row-walking unswizzler uses, with a test in `tests/d3d8_smoke` that swizzles an image and checks both agree texel for texel. Swizzling is the Xbox's, not Valve's. |
| Sampling swizzled textures in the executor | **fixed upstream**, `src/kernel/nv2a_pb_exec.c` | Three separate refusals, all silent: a stage needed a pitch to be valid and a swizzled texture has none; a swizzled texture's size is in the format word rather than `SET_TEXTURE_IMAGE_RECT`, which this title sends once against 3,176 format sets; and its coordinates are normalised rather than in texels. Every one of them ends in a flat quad. Nothing HL2-specific -- swizzled is the Xbox default. |
| Vertex attribute slot 9 as texcoord 0 | **fixed upstream**, `src/kernel/nv2a_pb_exec.c` | True only for a title that follows the convention. HL2's menu vertex is position, colour, texcoord at stride 24 with texcoords in slot 7. The fallback takes the first float2, unambiguous beside a float3 and a D3DCOLOR. This is the pushbuffer twin of the FVF position bug NoRain211 fixed in the D3D8 path (#23). |
| `xbox_GetDisplayFramebuffer` | **added upstream**, `src/kernel/xbox_memory_layout.h` | There was a setter and no getter, so every reader of guest pixels carried a hardcoded address instead. A title that moves its framebuffer -- most of them, once it owns one -- then leaves that constant on uninitialised memory, and the dump comes back as noise rather than empty, which reads as the title rendering garbage. |
